...
def deal_card():
    """Retorna carta aleatória do baralho"""
    cards = [11, 2, 3, 4, 5, 6, 7, 8, 9, 10, 10, 10, 10]
...
def calculate_score(score):
    """Vai receber uma lista de cartas e retornar a sua pontuação daquelas cartas."""
    ...
    if sum(score) > 21 and 11 in score:
        score.remove(11)
        score.append(1)
        return sum(score)
...
def play_game():
    ...
    while not game_over:        
        ...
        print(f"  Player: {user_cards} = {user_score}  \n")
        ...
        # then use the deal_card() function to add another card to the user_cards List. If no, then the game has ended.
            ...
            hit_or_stand = input('  [H]it or [S]tand?  ').lower()
    ...
    who_won = compare(user_score, computer_score)
    print(f"  Player >> {user_score} = {user_cards} \n")
    print(f"  Dealer >> {computer_score} = {computer_cards} \n")
    print(f"  {who_won}  \n")
...
while input("  Do you want to play a game of Blackjack? Type [Y]es or [N]o  ").lower() == 'y':
    os.system('clear')
    play_game()
